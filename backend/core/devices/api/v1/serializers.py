...
class DeviceManagementSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            ...
            physical_information_instance = (
                models.DevicePhysicalInformation.objects.create(
                    **physical_information_data
                )
            )

            camera_instance = models.Camera.objects.create(**camera_data)
            display_instance = models.Display.objects.create(**display_data)
            graphic_processor_instance = models.GraphicProcessor.objects.get_or_create(
                graphic_processor_name=graphic_processor["graphic_processor_name"],
                defaults=graphic_processor,
            )[0]
            processor_instance = models.Processor.objects.get_or_create(
                processor_name=processor["processor_name"], defaults=processor
            )[0]
            ...
            hardware_information_instance = (
                models.DeviceHardwareInformation.objects.create(
                    **hardware_information_data
                )
            )

            operating_system_instance = models.OperatingSystem.objects.get_or_create(
                os_name=operating_system["os_name"], defaults=operating_system
            )[0]
            software_information_data["os"] = operating_system_instance
            software_information_instance = (
                models.DeviceSoftwareInformation.objects.create(
                    **software_information_data
                )
            )

            features_instance = models.DeviceFeatures.objects.create(**features_data)
            category_instance = models.DeviceCategory.objects.get_or_create(

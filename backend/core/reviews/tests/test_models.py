import pytest
from reviews.tests.fixtures import (
    graphic_processor_review,
    device_review,
    processor_review,
...
class TestReviewModels:
    ...
    def test_str_representation(
        ...
        assert (
            str(processor_review) == f"{processor_review.target.processor_name} Review"
        )
        assert (
            str(graphic_processor_review)
            == f"{graphic_processor_review.target.graphic_processor_name} Review"
        )
        assert (
            str(operating_system_review)
            == f"{operating_system_review.target.os_name} Review"
        )

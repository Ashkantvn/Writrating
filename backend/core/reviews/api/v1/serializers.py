...
class AddReviewSerializer(serializers.Serializer):
    ...
    target_type = serializers.ChoiceField(
        choices=["Device", "Processor", "GraphicsProcessor", "OperatingSystem"]
    )
    ...
    def validate(self, attrs):
        ...
        if (
            target_type == "Device"
            and not DeviceModels.Device.objects.filter(slug=target).exists()
        ):
            raise exceptions.NotFound("Device does not exist")
        elif (
            target_type == "Processor"
            and not DeviceModels.Processor.objects.filter(slug=target).exists()
        ):
            raise exceptions.NotFound("Processor does not exist")
        elif (
            target_type == "GraphicsProcessor"
            and not DeviceModels.GraphicProcessor.objects.filter(slug=target).exists()
        ):
            raise exceptions.NotFound("GraphicsProcessor does not exist")
        elif (
            target_type == "OperatingSystem"
            and not DeviceModels.OperatingSystem.objects.filter(slug=target).exists()
        ):
...
class EditReviewSerializer(serializers.Serializer):
    ...
    def update(self, instance, validated_data):
        ...
        instance.buying_worth = validated_data.get(
            "buying_worth", instance.buying_worth
        )

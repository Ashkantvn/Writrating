...
class TestAccountModels(TestCase):
    ...
    def test_create_user(self):
        user = self.User.objects.create_user(
            email="test@test.com", password="waoojfdoiij@3324ADF"
        )
    ...
    def test_create_super_user(self):
        user = self.User.objects.create_superuser(
            email="test@test.com", password="wodojfoadsji@#o5u08ASDF"
        )
        ...
        with self.assertRaises(ValueError):
            self.User.objects.create_superuser(
                email="test@test.com", password="pwijsd@#$123SF", is_superuser=False
            )

    # test for create profile model after user creation
    def test_user_create_profile(self):
        user = self.User.objects.create_user(
            email="test@test.com", password="adjfapwo@#123AFDADS"
        )
    ...
    def test_create_password_recovery_code(self):
        user = self.User.objects.create_user(
            email="test@test.com", password="adjfoij@#$542234EWRw"
        )

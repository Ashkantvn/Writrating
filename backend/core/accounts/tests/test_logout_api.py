...
class TestAccountLogoutAPI:
    ...
    def test_POST_logout_user_401(self):
        ...
        response = client.post(
            url, data={"access_token": "token", "refresh_token": "refresh_token"}
        )
    ...
    def test_POST_logout_user_400(self, authenticated_client):
        ...
        response = authenticated_client.post(
            url, data={"access_token": "", "refresh_token": ""}
        )
        assert response.status_code == 400

        response = authenticated_client.post(
            url, data={"access_token": "token", "refresh_token": ""}
        )
        assert response.status_code == 400

        response = authenticated_client.post(
            url, data={"access_token": "", "refresh_token": "token"}
        )
        assert response.status_code == 400

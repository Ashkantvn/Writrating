...
class TestCompareDevices:
    ...
    def test_compare_devices_is_resolve(self):
        url = reverse(
            "interactions:compare-devices", kwargs={"slug1": "test1", "slug2": "test2"}
        )
        view_class = resolve(url).func.view_class
        assert view_class == views.CompareDevicesAPIView

    def test_GET_compare_devices_status_200(
        self, authenticated_client, first_device, second_device
    ):
        client = authenticated_client
        url = reverse(
            "interactions:compare-devices",
            kwargs={"slug1": first_device.slug, "slug2": second_device.slug},
        )
    ...
    def test_GET_compare_devices_status_400(self, authenticated_client, first_device):
        ...
        url = reverse(
            "interactions:compare-devices",
            kwargs={"slug1": first_device.slug, "slug2": first_device.slug},
        )
        response = client.get(url)
